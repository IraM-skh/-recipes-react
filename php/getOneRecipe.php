...
$recipesDB = $mysql->query("SELECT * FROM `recipes` WHERE `id` = $idRecipe");
$recipesArrForObj =[];
$recipes =[];
...
while($row = mysqli_fetch_array($recipesDB)){
   ...
   $recipesArrForObj["author"] =$row["author_login"];
   ...
   //разделяем ид тегов 
    ...
    $recipe_stepsDB = $mysql->query("SELECT * FROM `recipe_steps` WHERE `id_recipe` = '$id_recipe'");
