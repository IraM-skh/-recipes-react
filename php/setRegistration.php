...
$id_hash ='';
$time = 3600*24*30;
$userLoginDB = $mysql->query("SELECT * FROM `users` WHERE `login`='$login'");
$userMailDB = $mysql->query("SELECT * FROM `users` WHERE `e_mail`='$e_mail'");
...
if($row_login_cnt != 0){
    ...
    $mysql->query("INSERT INTO `users`(`login`, `password`, `e_mail`) VALUES ('$login','$password', '$e_mail')");
